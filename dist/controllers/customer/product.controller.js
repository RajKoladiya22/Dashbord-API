...
const getCustomerProductsByCustomerId = async (req, res, next) => {
    ...
    const user = req.user;
    if (!user) {
        return (0, responseHandler_1.sendErrorResponse)(res, 401, 'Unauthorized');
    }
    const adminId = user.role === 'admin' ? user.id : user.adminId;
    if (!adminId) {
        return (0, responseHandler_1.sendErrorResponse)(res, 403, 'Forbidden: Admin ID not found');
    }
    try {
        ...
        return (0, responseHandler_1.sendSuccessResponse)(res, 200, 'Customer products fetched', { history });
    }
    catch (error) {
